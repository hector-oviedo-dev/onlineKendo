...
if ($valid) {
	...
	if ($auth && $logged) {
	  ob_end_clean();
		$result = array ();
			...
			$consult_str = "SELECT * FROM plays,users WHERE users.ID = plays.UID";
      ...
      $jugadasentotal;
	    while($row = mysqli_fetch_array($consult)) {
	      ...
	      $play = array ('GID' => $play_gid,'NAME' => $play_name,'BET' => $play_bet,'PAY' => $play_pay,'JACKPOT' => $play_jackpot,'BEFORE' => $play_ucb,'AFTER' => $play_uca,'FECHA' => $play_fecha);
	...
	echo '
	<div class="jumbotron">
	<h2>Resultado de todos los juegos</h2>
	<table class="table">
		<tbody>
    <tr class="success">
      <td>Win</td>
